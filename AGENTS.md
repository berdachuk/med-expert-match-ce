...
These are regenerated by `scripts/sync-memory-index.sh` from the registries and record files below. To change their content, edit the source registries/records and re-run the script. `scripts/sync-memory-index.sh --check` is a CI assertion that they are in sync.

| File | Source |
|------|--------|
| `activeContext.md` | `records/active/M*.md` + `registry/risk.jsonl` + `registry/scn.jsonl` |
| `progress.md` | `records/progress/M*.md` (sorted) |
...
`registry/` holds one JSONL file per stable-ID kind: `req.jsonl`, `nfr.jsonl`, `scn.jsonl`, `test.jsonl`, `dec.jsonl`, `risk.jsonl`, `task.jsonl`. Schemas are defined in `registry/SCHEMA.md`. **To mint a new ID:** read the registry, take `max+1`, append exactly one line. If a merge conflict occurs on the last line, re-read and recompute — conflicts collapse to "who owns the last line" and are trivially resolvable. Never edit an existing line.
...
### Module locks (`.agents/memory-bank/locks/`)

`locks/<module>.md` records which agent/branch currently owns a module. Before editing any coupled file pair (e.g. a prompt `.st` and `core/util/LlmResponseSanitizer.java` — the M130-class lockstep risk), an agent MUST hold that module's lock. See `locks/README.md` for format, the coupled-file-pair table, and acquisition rules. Locks are advisory but enforced by the `code-style` and `security-check` skills.

### Worktree scratchpads (git-ignored)

`.agents/memory-bank/worktrees/<branch-slug>/` is a per-worktree scratchpad for "Current Focus" drafts and open questions that should never be merged to `develop`. It is in `.gitignore`. On merge, promote only the `records/` and `registry/` files into the canonical memory bank.
